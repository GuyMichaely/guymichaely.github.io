...
The `Private mode` button in the sidebar keeps the real portfolio loaded — so every breakdown shows true proportions — while blanking every absolute magnitude, for demoing pie breakdowns without revealing how much is held. Dollar amounts, share quantities, and account balances render blank everywhere they would otherwise appear: the net-worth and cash-flow panel, concentration totals, pie legend amounts and hover tooltips, the sale planner (curve and companion axis labels, profit/margin-paydown figures), the rebalance summary and transaction columns, the lots table's value/profit/shares columns, and the data-management balance inputs (which also become disabled). What stays visible is everything that conveys proportion rather than magnitude: pie slice sizes, all percentages, lot counts, and per-share market prices (public data that reveals nothing about holding size). Interactive inputs that would otherwise echo a magnitude are blanked and disabled so a blank can't be edited in: account-balance inputs, the sale-planner amount/profit/margin-paydown inputs, the rebalance gross-gain/gross-loss limit inputs, and — in the Record-trades sell picker — a selected lot's share and dollar-volume inputs (so checking a lot no longer reveals its size). In the sector-exposure targets the per-row `$`/`%` toggle is forced to `%` and the `$` button disabled, so targets are always shown and edited as percentages rather than dollar amounts. The FIRE panel additionally blanks its two derived percentages (greatest survivable drop / addition needed, and minimum supportable FIRE rate) and disables the FIRE-rate input, since those convey how close the portfolio is to its goal. Like demo mode, entering prompts for a password required to exit and the mode persists across reloads; the password check is a client-side convenience against shoulder-surfing, not real security. Private mode is independent of demo mode — it changes only how values are displayed, never the underlying data or the cache — and the two can be active at once.
